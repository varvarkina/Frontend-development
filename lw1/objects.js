...
function renamePresentationTitle(newTitle, presentation) {
    return {
        ...presentation,
        title: newTitle
    };
...
function changeSlidePosition(newIndex, presentation) {
...
function addContentToSlide(presentation, slideId, element) {
...
function removeContentFromSlide(presentation, slideId, elementId) {
    const updatedSlideCollection = presentation.slideCollection.map(slide => {
        if (slide.id === slideId) {
...
function changeElementPosition(slideId, elementId, newPosition, presentation) {
    return {
        ...presentation,
...
function changeElementSize(slideId, elementId, newSize, presentation) {
    return {
        ...presentation,
        slideCollection: presentation.slideCollection.map(slide => {
            if (slide.id === slideId) {
...
function changeSlideText(slideId, elementId, newValue, presentation) {
    return {
        ...presentation,
        slideCollection: presentation.slideCollection.map(slide => {
            if (slide.id === slideId) {
...
function changeFontFamily(slideId, elementId, newFontFamily, presentation) {
    return {
        ...presentation,
        slideCollection: presentation.slideCollection.map(slide => {
            if (slide.id === slideId) {
...
function updateSlideBackground(presentation, slideId, newBackground) {
    const updatedSlideCollection = presentation.slideCollection.map(slide => {
        if (slide.id === slideId) {
            return {
                ...slide,
...
const presentationWithNewTitle = renamePresentationTitle('Новое название презентации', presentation);
...
const presentationRemoveText = removeContentFromSlide(presentationWithText, 'slide1', presentationWithText.slideCollection[0].content[0].id);
const presentationRemoveImage = removeContentFromSlide(presentationRemoveText, 'slide2', presentationRemoveText.slideCollection[1].content[0].id);
console.log(`Presentation without removed content: ${JSON.stringify(presentationRemoveImage)}\n`);
const presentationChangeElPos = changeElementPosition('slide1', presentationWithText.slideCollection[0].content[0].id, { x: 100, y: 200 }, presentationWithText);
console.log(`Changing element position: ${JSON.stringify(presentationChangeElPos)}\n`);
const presentationChangeElSize = changeElementSize('slide2', presentationWithImage.slideCollection[1].content[0].id, { width: 300, height: 200 }, presentationWithImage);
console.log(`Changing element size: ${JSON.stringify(presentationChangeElSize)}\n`);
const presentationWithNewText = changeSlideText('slide1', presentationWithText.slideCollection[0].content[0].id, 'New text', presentationWithText);
console.log(`Changing slide text: ${JSON.stringify(presentationWithNewText)}\n`);
const presentationWithNewFontFamily = changeSlideText('slide1', presentationWithText.slideCollection[0].content[0].id, 'Oxygen', presentationWithText);
console.log(`Changing font family: ${JSON.stringify(presentationWithNewFontFamily)}\n`);
// const presentationWithBackground = changeSlideText(
//     'slide1',
